...
pub fn get_ip_from_do_load_balancer_api_output(
    json_content: &str,
    ...
    match res_load_balancer {
        Ok(lb) => match Ipv4Addr::from_str(&lb.load_balancer.ip) {
                    ...
                    Err(e) => {
                        Err(SimpleError::new(
                            ...
                            Some(
                                format!("Info returned from DO API is not a valid IP, received '{:?}' instead. {:?}", &lb.load_balancer.ip, e),
                            ),
                        ))
                    }
                },
        Err(_) => {
            Err(SimpleError::new(
                SimpleErrorKind::Other,
                Some(
                    format!("Error While trying to deserialize json received from Digital Ocean Load Balancer API"),
                ),
            ))
        }
    }
