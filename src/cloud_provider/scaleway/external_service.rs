...
impl Create for ExternalService {
    fn on_create(&self, target: &DeploymentTarget) -> Result<(), EngineError> {
        info!("SCW.external_service.on_create() called for {}", self.name());
    ...
    fn on_create_error(&self, target: &DeploymentTarget) -> Result<(), EngineError> {
        warn!("SCW.external_service.on_create_error() called for {}", self.name());
...
impl Pause for ExternalService {
    fn on_pause(&self, _target: &DeploymentTarget) -> Result<(), EngineError> {
        info!(
            "SCW.external_service.on_pause() called for {}, doing nothing",
            self.name()
        );
        Ok(())
    }
    ...
    fn on_pause_error(&self, _target: &DeploymentTarget) -> Result<(), EngineError> {
        warn!("SCW.external_service.on_pause_error() called for {}", self.name());
        Ok(())
    }
}
...
impl Delete for ExternalService {
    fn on_delete(&self, target: &DeploymentTarget) -> Result<(), EngineError> {
        info!("SCW.external_service.on_delete() called for {}", self.name());
    ...
    fn on_delete_error(&self, target: &DeploymentTarget) -> Result<(), EngineError> {
        warn!("SCW.external_service.on_delete_error() called for {}", self.name());
