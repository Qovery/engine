use crate::cmd::utilities::{exec_with_envs_and_output, exec_with_output, CmdError};
use std::io::Error;
...
pub fn doctl_do_registry_login(token: &str) -> Result<(), CmdError> {
    ...
    if output_string.contains("401") {
        return Err(CmdError::Unexpected(
            Error::new(std::io::ErrorKind::PermissionDenied, output_string).to_string(),
        ));
    }
    Ok(())
}

pub fn doctl_do_registry_get_repository(token: &str) -> Result<(), CmdError> {
    let mut output_string = String::new();
...
pub fn doctl_do_registry_create(token: &str) -> Result<(), CmdError> {
    let _ = doctl_exec_with_output(
