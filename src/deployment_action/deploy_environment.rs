...
impl DeploymentThreadsPool {
        ...
        Task: FnMut() -> Result<(), Err> + Send,
    {
        ...
        thread::scope(|scope| {
            ...
            let mut await_deployment_slot = |active_threads: &mut VecDeque<ScopedJoinHandle<_>>| {
            ...
            for mut task in tasks {
                ...
                await_deployment_slot(&mut active_threads);
...
mod test {
    ...
    fn test_deployment_thread_pool_error_cancelling_other_tasks() {
        // setup:
        const TASKS_COUNT: usize = 10;
        const FAILING_TASK_NUMBER: usize = 5;
        type MaxParallelDeploy = usize;
        let test_cases: Vec<MaxParallelDeploy> = vec![1, 3, 5];
        ...
        for tc in test_cases {
            ...
            let ret = pool.run(tasks, || false, NonZeroUsize::new(tc).unwrap());
