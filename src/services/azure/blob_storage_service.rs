...
pub enum BlobStorageServiceError {
    ...
    #[error("Cannot check if bucket `{bucket_name}` exists: {raw_error_message:?}")]
    ...
    #[error("Cannot get object properties `{object_key}` from bucket `{bucket_name}`: {raw_error_message:?}")]
    ...
    #[error("Cannot list buckets from account `{account}`: {raw_error_message:?}")]
    CannotListBuckets { account: String, raw_error_message: String },
}

#[cfg_attr(test, faux::create)]
pub struct BlobStorageService {
    account: String,
    storage_credentials: Arc<StorageCredentials>,
}

#[cfg_attr(test, faux::methods)]
impl BlobStorageService {
    pub fn new(account: &str, access_key: &str) -> Self {
        let storage_credentials = StorageCredentials::access_key(account.to_string(), access_key.to_string());

        Self {
            account: account.to_string(),
            storage_credentials: Arc::new(storage_credentials),
        }
    }

    fn client(&self) -> ClientBuilder {
        ClientBuilder::new(&self.account, (*self.storage_credentials).clone()) // TODO(benjaminch): to be improved
    }

    pub fn bucket_exists(&self, bucket_name: &str, bucket_location: &AzureStorageRegion) -> bool {
    ...
    pub fn delete_bucket(&self, bucket_name: &str) -> Result<(), BlobStorageServiceError> {
        let client = self.client();
    ...
    pub fn update_bucket(
        ...
        let client = self.client();
        let _container_client = client.container_client(bucket_name);
    ...
    pub fn get_object(&self, bucket_name: &str, object_key: &str) -> Result<BucketObject, BlobStorageServiceError> {
        let client = self.client();
        let container_client = client.container_client(bucket_name);
    ...
    pub fn put_object(
        ...
        match block_on(blob_client.put_block_blob(content).metadata(&metadata).into_future()) {
            Ok(_) => self.get_object(bucket_name, object_key),
    ...
    pub fn delete_object(&self, bucket_name: &str, object_key: &str) -> Result<(), BlobStorageServiceError> {
        let client = self.client();
        let container_client = client.container_client(bucket_name);
        let blob_client = container_client.blob_client(object_key);
