...
use crate::helpers;
use crate::helpers::azure::{AZURE_SELF_HOSTED_DATABASE_DISK_TYPE, azure_infra_config, clean_environments};
...
fn deploy_an_environment_with_3_databases_and_3_apps() {
    ...
    engine_run_test(|| {
        ...
        let cluster_id = secrets.AZURE_TEST_CLUSTER_LONG_ID.expect("AZURE_TEST_CLUSTER_LONG_ID");
        ...
        let infra_ctx_for_deletion = azure_infra_config(
        ...
        let environment = helpers::database::environment_3_apps_3_databases(
            &context,
            None,
            AZURE_SELF_HOSTED_DATABASE_DISK_TYPE.to_k8s_storage_class().as_str(),
            Kind::Azure,
        );
...
fn deploy_an_environment_with_db_and_pause_it() {
    ...
    engine_run_test(|| {
        ...
        let ret = get_pods(&infra_ctx, ProviderKind::Azure, &environment, &environment.databases[0].long_id);
        assert!(ret.is_ok());
...
fn postgresql_deploy_a_working_environment_and_redeploy() {
    ...
    engine_run_test(|| {
        ...
        let infra_ctx_for_redeploy = azure_infra_config(
            &target_cluster_azure_test,
        ...
        let database_host = get_svc_name(DatabaseKind::Postgresql, Kind::Azure).to_string();
        ...
        environment.databases = vec![Database {
            ...
            database_disk_type: AZURE_SELF_HOSTED_DATABASE_DISK_TYPE.to_k8s_storage_class(),
            ...
            blueprint_owns_external_name: false,
        }];
...
fn private_mysql_v9_deploy_a_working_dev_environment() {
    test_mysql_configuration("9.7-oracle", function_name!(), CONTAINER, false);
}
...
fn public_mysql_v9_deploy_a_working_dev_environment() {
    test_mysql_configuration("9.7-oracle", function_name!(), CONTAINER, true);
}
