...
#[named]
#[test]
fn deploy_container_with_ndots_on_aws_eks() {
    engine_run_test(|| {
        let span = span!(Level::INFO, "test", name = function_name!());
        let _enter = span.enter();
        ...
        let pods = get_pods(&infra_ctx, Kind::Aws, &environment, &environment.containers[0].long_id)
...
#[named]
#[test]
fn deploy_container_with_none_ndots_on_aws_eks() {
    engine_run_test(|| {
        let span = span!(Level::INFO, "test", name = function_name!());
        let _enter = span.enter();
