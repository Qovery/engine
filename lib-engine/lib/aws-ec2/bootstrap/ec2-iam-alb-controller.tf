resource "aws_iam_policy" "aws_load_balancer_controller_policy" {
  name = "qovery-aws-alb-controller-${var.kubernetes_cluster_id}"
...
resource "aws_iam_role" "aws_load_balancer_controller" {
  name = "qovery-ec2-alb-controller-${var.kubernetes_cluster_id}"
...
resource "aws_iam_role_policy_attachment" "aws_load_balancer_controller" {
  ...
  // using the same role as the EBS CSI driver as EC2 can only use one
  role       = aws_iam_role.aws_ebs_csi_driver.name
}