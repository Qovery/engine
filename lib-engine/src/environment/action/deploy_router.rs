...
fn ensure_gateway_certificate_ref(
    ...
    let listener = listeners
        .iter_mut()
        .find(|listener| listener.get("name").and_then(|value| value.as_str()) == Some(listener_name))
        .ok_or_else(|| {
            CommandError::new_from_safe_message(format!(
    ...
    let already_present = cert_refs_array.iter().any(|reference| {
        reference.get("name").and_then(|value| value.as_str()) == Some(secret_name)
            && reference.get("namespace").and_then(|value| value.as_str()) == Some(secret_namespace)
    });
