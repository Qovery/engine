    ...
    fn on_create(&self, target: &DeploymentTarget) -> Result<(), Box<EngineError>> {
        ...
        let run = |logger: &EnvProgressLogger, _: ()| -> Result<(), Box<EngineError>> {
            ...
            let chart = ChartInfo {
                ...
                // The Qovery-authored official-image charts (PostgreSQL 18+, MongoDB 8.3+, Redis 8+) ship
                // as version 0.1.0; comparing that against the Bitnami thresholds would force a reinstall
                // on every redeploy, so the official families opt out with None.
                reinstall_chart_if_installed_version_is_below_than: match T::db_type() {
                    service::DatabaseType::PostgreSQL if !self.is_bitnami() => None,
                    service::DatabaseType::PostgreSQL => Some(Version::new(12, 5, 1)),
                    service::DatabaseType::MongoDB if !self.is_bitnami() => None,
                    service::DatabaseType::MongoDB => Some(Version::new(13, 13, 1)),
                    service::DatabaseType::MySQL if !self.is_bitnami() => None,
                    service::DatabaseType::MySQL => Some(Version::new(9, 10, 1)),
                    service::DatabaseType::Redis if !self.is_bitnami() => None,
