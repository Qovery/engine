...
use crate::environment::models::database::{
    Container, Database, DatabaseType, Managed, MongoDB, MySQL, PostgresSQL, Redis,
};
    ...
    Database<AWS, Managed, T>: Service,
{
    ...
    fn to_tera_context_for_aws_managed(
        ...
        context.insert("aws_apn_id", kubernetes.context().aws_apn_id().tag_value());
        ...
        context.insert("kubernetes_cluster_long_id", &kubernetes.long_id().to_string());
