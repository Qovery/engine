use crate::helm::{
    ...
    QoveryGatewayClass, QoveryPriorityClass, UpdateStrategy, get_engine_helm_action_from_location,
};
...
pub fn kapsule_helm_charts(
        ...
        .unwrap_or(false)
    {
        ...
        qovery_cluster_gateway = Some(
            QoveryClusterGatewayChart::new(
                ...
                LoadBalancer::Scaleway(ScalewayLoadBalancer {
                    size: Some(
                        chart_config_prerequisites
                            .cluster_advanced_settings
                            .load_balancer_size
    ...
    let mut level_4: Vec<Box<dyn HelmChart>> = vec![Box::new(external_dns_secret)];
    ...
    if let Some(chart) = qovery_cluster_gateway {
        level_4.push(Box::new(chart));
    }

    let level_5: Vec<Box<dyn HelmChart>> = vec![Box::new(external_dns)];

    let mut level_6: Vec<Box<dyn HelmChart>> = Vec::with_capacity(2);
    // Add Nginx
