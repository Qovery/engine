...
resource "google_service_account_iam_binding" "external_secrets_operator_workload_identity" {
  service_account_id = google_service_account.eso_operator_service_account.name
...
# Secret manager accessor can be assigned only at project level
resource "google_project_iam_binding" "external_secrets_operator_secret_access" {
  project = var.project_id
  role    = "roles/secretmanager.secretAccessor"
  members = [
    "serviceAccount:${google_service_account.eso_operator_service_account.email}",
  ]
}
