...
resource "aws_docdb_cluster" "documentdb_cluster" {
  ...
  tags = {
    ...
    q_project_id = var.q_project_id
    database_identifier = var.documentdb_identifier
    {% if service_info["snapshot"] and service_info["snapshot"]["snapshot_id"] %}meta_last_restored_from = var.snapshot_identifier{% endif %}
  }
  ...
  {%- if service_info["snapshot"] and service_info["snapshot"]["snapshot_id"] %}
  # Snapshot
