...
use qovery_engine::infrastructure::models::kubernetes::azure::AzureStorageType;
...
#[named]
#[test]
fn azure_aks_deploy_container_with_ndots() {
    let test_name = function_name!();
    engine_run_test(|| {
        let span = span!(Level::INFO, "test", name = test_name);
        let _enter = span.enter();
        ...
        let pods = get_pods(&infra_ctx, Kind::Azure, &environment, &environment.containers[0].long_id)
...
#[named]
#[test]
fn azure_aks_deploy_container_with_none_ndots() {
    let test_name = function_name!();
    engine_run_test(|| {
        let span = span!(Level::INFO, "test", name = test_name);
        let _enter = span.enter();
