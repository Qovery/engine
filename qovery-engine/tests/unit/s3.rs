...
use test_utilities::aws::{AWS_ACCESS_KEY, AWS_DEFAULT_REGION, AWS_KEY_ID, AWS_REGION_FOR_S3};
...
fn delete_s3_bucket() {
    ...
    let creation = s3::create_bucket(AWS_KEY_ID, AWS_ACCESS_KEY, bucket_name);
    match creation {
        Ok(out) => println!("Yippee Ki Yay"),
        Err(e) => println!("While creating the bucket {}", e),
    }

    let delete = delete_bucket(AWS_KEY_ID, AWS_ACCESS_KEY, bucket_name);
    match delete {
        Ok(out) => println!("Yippee Ki Yay"),
