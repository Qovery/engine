...
use tokio::io::Error;
use crate::cmd::{kubectl_exec_create_namespace,kubectl_exec_delete_secret};

// generate the kubernetes config path
pub fn get_kubernetes_config_path(workspace : &str, kubernetes: &dyn Kubernetes, environment: &Environment ) -> Result<String, Error> {
    let aws = kubernetes
        .cloud_provider()
        .as_any()
        .downcast_ref::<AWS>()
...
pub fn delete_terraform_tfstate_secret(kubernetes: &dyn Kubernetes, environment: &Environment, workspace_dir : &str) -> Result<(), Error> {
    let aws = kubernetes
        .cloud_provider()
        .as_any()
        .downcast_ref::<AWS>()
