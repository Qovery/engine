    ...
    fn on_create(&self, target: &DeploymentTarget) -> Result<(), ServiceError> {
        ...
        match target {
            ...
            DeploymentTarget::SelfHosted(kubernetes, environment) => {
                ...
                {
                    return Err(ServiceError::OnCreateFailed(ErrorContext{ item_type: ItemType::Service, item_id: self.id.to_string() }));
                }
                ...
                match crate::cmd::kubectl_exec_is_application_ready_with_retry(
                    ...
                    Ok(Some(true)) => {}
                    _ => return Err(ServiceError::OnCreateFailed(ErrorContext{ item_type: ItemType::Service, item_id: self.id.to_string() })),
                }
            }
        }
